process RENAME_GENOMES {
    ...
    bioawk  -v id=$meta.id -c fastx 'BEGIN{print "previous_IDs", "EXOGAP_IDs"} {print \$name, id"-seq"NR}' $unmodified_genome > ${meta.id}.tsv

    // Modification of genome file
    bioawk -v id=$meta.id -c fastx '{print ">"id"-seq"NR; print \$seq}' $unmodified_genome |fold -w 60 > $genome
    """
}
