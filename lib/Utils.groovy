...
class Utils {
    ...
    // // Update the value of a top-level key
    public static LinkedHashMap updateTopLevelKey(LinkedHashMap map, String key, def newValue) {
        map[key] = newValue

        return map
    }

    // Update the value of a subkey in a nested map
    public static LinkedHashMap updateSubkey(LinkedHashMap map, String key, String subkey, def newValue) {
        if (map[key] instanceof Map) {
            map[key][subkey] = newValue
        } else {
            throw new IllegalArgumentException("The specified key does not map to a nested map")
        }
    ...
    public static DataflowBroadcast createChannelToGatherFiles(String name, DataflowBroadcast channel) {

        return channel.map { file -> [name, file]}.groupTuple()

    }


    public static DataflowBroadcast combineChannels(List<Channel> channel_list, String id, String output) {
        return channel_list[0].concat(channel_list[1]).map { name, file -> ['group', file] }.groupTuple().map { group, files -> [id, files, output] }
    }

    public static Channel joinGenomesAndOuts(Channel outs) {
        // Example implementation: just return the channel for now
        return outs
    }
}
