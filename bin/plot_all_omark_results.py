...
def create_df_from_results(file_list):
    ...
    for path in file_list:
        # If the provided path is a direct .sum file, parse it
        if os.path.isfile(path) and path.lower().endswith('.sum'):
            main_data, cont_data = parse_sum_file(path)
            all_main_data.append(main_data)
            all_cont_data += cont_data
            continue

        # If the provided path is a directory, look for .sum files inside
        if os.path.isdir(path):
            file_found = False
            for file in os.listdir(path):
                if file.lower().endswith('.sum'):
                    ...
                    all_cont_data += cont_data
            if not file_found:
                # no .sum file found for this path
                pass
        # otherwise ignore non-existing or non-regular paths
...
if __name__=='__main__':
    ...
    # Accept either .sum files placed directly in the input folder or subfolders
    entries = [os.path.join(input_folder, x) for x in os.listdir(input_folder)]
    # Also include the input folder itself in case it directly is a .sum file path
    if input_folder.lower().endswith('.sum') and os.path.isfile(input_folder):
        entries.insert(0, input_folder)
    main_df, cont_df =  create_df_from_results(entries)
    if mapping_file:
